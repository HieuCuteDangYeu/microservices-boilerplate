...
export class RedisCallSessionRepository implements ICallSessionRepository {
  ...
  async save(session: CallSession): Promise<CallSession> {
    await this.redis.set(this.key(session.roomId), JSON.stringify(session), 'EX', 60 * 60 * 6)
    return session
  }
  ...
  async updateStatus(roomId: string, status: CallSession['status']): Promise<CallSession | null> {
