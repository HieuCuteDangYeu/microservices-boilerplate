...
describe('OutboxDispatcherService', () => {
  ...
  it('coalesces multiple outbox-created wake signals into an initial and follow-up dispatch', async () => {
    const { service, execute } = createService();
  ...
  it('runs a follow-up drain when a trigger arrives while a drain is scheduled', async () => {
    const { service, execute } = createService({
      results: [
        { claimed: 0, published: 0, failed: 0 },
